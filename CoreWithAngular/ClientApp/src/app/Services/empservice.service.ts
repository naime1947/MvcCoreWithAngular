...
export class EmployeeService {
    ...
    getEmployeeById(id: number) {
        return this._http.get(this.myAppUrl + 'api/Employee/Details/' + id).map(res => res.json())
            .catch(this.errorHandler);
    }

    saveEmployee(employee) {
        return this._http.put(this.myAppUrl + 'api/Employee/Create', employee).map(res => res.json())
            .catch(this.errorHandler);
    }
